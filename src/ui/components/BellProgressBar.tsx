...
function BellMarker({ position, label }: BellMarkerProps) {
  return (
    ...
    >
      <Bell size={18} className="w-3.5 h-3.5 sm:w-[18px] sm:h-[18px] landscape:w-3 landscape:h-3 text-yellow-500 fill-yellow-500" />
      <div
        className="mt-0.5 text-yellow-500/90 font-mono font-semibold text-[8px] sm:text-[10px] landscape:text-[6px] tracking-widest uppercase"
      >
        {label}
      </div>
    </div>
...
export function BellProgressBar({
  ...
  return (
    <div className="w-full max-w-5xl">
      <div className="relative h-12 mb-4 landscape:h-8 landscape:mb-1">
        {bells.map((bell) => (
          <BellMarker
            key={bell.label}
